...
def run_all_methods(
    ...
    trend_check = parallel_trends_test(df, intervention_time)
    ...
    for name, cls in ESTIMATORS.items():
        est = cls()
        est.fit(df, treated_unit=treated_unit, intervention_time=intervention_time, donor_units=donor_pool)
        eff = est.effect()
        results[name] = eff
        try:
            figs[name] = est.plot()
...
def run_placebo_suite(
    ...
    for name, cls in ESTIMATORS.items():
        it = in_time_placebo(
            cls, df, treated_unit, intervention_time, fake_intervention_time, threshold,
            donor_units=donor_pool,
        )
        isp = in_space_placebo(cls, df, treated_unit, intervention_time, threshold, donor_units=donor_pool)
        suite[name] = {
            "in_time": it,
            "in_space": isp,
...
def render_html_report(comparison: dict, placebo: dict, out_path: str, title: str = "Causal Inference Toolkit Report") -> str:
    rows = []
    for name, eff in comparison["results"].items():
        rows.append(
            f"<tr><td>{name}</td><td>{eff.point_estimate:.3f}</td>"
            f"<td>[{eff.ci_lower:.3f}, {eff.ci_upper:.3f}]</td>"
            f"<td>{eff.p_value:.4f}</td>"
            f"<td>{placebo[name]['in_space_pass_rate']*100:.1f}%</td></tr>"
        )
    ...
    for name, ax in comparison["figs"].items():
        ...
        imgs_html += f"<h3>{name}</h3><img src='data:image/png;base64,{b64}' style='max-width:700px;'/><br/>"

    gt_html = ""
    if comparison["ground_truth_check"]:
        gt_rows = "".join(
            f"<tr><td>{k}</td><td>{v['estimate']:.3f}</td><td>{v['true_effect']:.3f}</td>"
            f"<td>{v['abs_error']:.3f}</td><td>{v['within_ci']}</td></tr>"
    ...
    <p>{comparison['trend_check'].message}</p>
    <p>Donor pool ({len(comparison['donor_pool'])} units): {', '.join(comparison['donor_pool'])}</p>
    ...
    <p><b>Disagreement:</b> spread={comparison['disagreement']['spread']:.3f},
    relative spread={comparison['disagreement']['relative_spread']:.2%}.
    {"Methods are in reasonable agreement." if comparison['disagreement']['relative_spread'] < 0.5 else
     "Methods disagree substantially — inspect assumptions (parallel trends for DiD, donor fit for synthetic control, model specification for BSTS) before trusting any single number."}</p>
